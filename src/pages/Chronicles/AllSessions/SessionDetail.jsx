    ...
    <main style={{ minHeight: "100%" }}>
      ...
      <section style={{ position: "relative", height: 320 }}>
        ...
        <div style={{ position: "absolute", bottom: 12, left: 16, right: 16, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div>
            <div style={{ opacity: 0.95 }}>{s.realDate} • {s.gameDate} • #{s.num}</div>
      ...
      {/* Content */}
      <section style={{ display: "grid", gridTemplateColumns: "minmax(0,1fr) 320px", gap: 24, padding: 24 }}>
            ...
            <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
              {s.participants.map((p) => (
                <figure key={p.id} style={{ margin: 0, textAlign: "center" }}>
                  <img src={p.portrait} alt={p.name} style={{ width: 72, height: 72, objectFit: "cover", borderRadius: 12 }} />
      ...
      <section style={{ padding: 24, display: "flex", justifyContent: "space-between" }}>
        {prev ? (
          <Link to={`/chronicles/sessions/${prev.id}`} style={{ textDecoration: "none", color: "#fff", background: "#1f2a44", padding: "8px 12px", borderRadius: 8 }}>Предыдущая сессия</Link>
        ) : <span />}
        {next ? (
          <Link to={`/chronicles/sessions/${next.id}`} style={{ textDecoration: "none", color: "#fff", background: "#1f2a44", padding: "8px 12px", borderRadius: 8 }}>Следующая сессия</Link>
        ) : <span />}
      </section>
    </main>
  );
...
function InfoCard({ title, value }) {
  return (
    <div style={{ background: "#121212", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 12, padding: 12, marginBottom: 12 }}>
