...
export default function CityDetail() {
  ...
  return (
    <main style={{ minHeight: "100%" }}>
      {/* Top */}
      <section style={{ position: "relative", height: "50vh", background: `url(${city.panorama}) center/cover no-repeat` }}>
        <div style={{ position: "absolute", inset: 0, background: "rgba(0,0,0,0.45)" }} />
        <div style={{ position: "absolute", bottom: 16, left: 24, right: 24, display: "flex", alignItems: "center", gap: 12, justifyContent: "space-between" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            <img src={city.flag} alt="flag" style={{ width: 64, height: 64, objectFit: "contain" }} />
            <div>
      ...
      {/* Three columns */}
      <section style={{ display: "grid", gridTemplateColumns: "320px minmax(0,1fr) 340px", gap: 24, padding: 24 }}>
        {/* Left */}
        <aside>
          <div>
            <div style={{ fontWeight: 700, marginBottom: 8 }}>Карта расположения</div>
            <img src={city.mapSmall} alt="map" style={{ width: "100%", borderRadius: 8, border: "1px solid rgba(255,255,255,0.1)" }} />
          </div>

          <div style={{ marginTop: 16 }}>
            <div style={{ fontWeight: 700, marginBottom: 8 }}>Информация</div>
            <InfoRow label="Население" value={city.info.population.toLocaleString()} />
            <InfoRow label="Правитель" value={city.info.ruler.name} avatar={city.info.ruler.portrait} />
          ...
          <div style={{ marginTop: 16 }}>
            ...
            <div style={{ display: "grid", gap: 8 }}>
              {city.districts.map((d) => (
                <div key={d.name} style={{ display: "flex", gap: 8, alignItems: "flex-start", background: "#111", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 8, padding: 10 }}>
                  <img src={d.icon} alt="icon" style={{ width: 20, height: 20 }} />
                  <div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(220px, 1fr))", gap: 8 }}>
              {city.buildings.map((b) => (
                <figure key={b.name} style={{ margin: 0 }}>
                  <img src={b.image} alt={b.name} style={{ width: "100%", height: 140, objectFit: "cover", borderRadius: 8 }} />
            ...
            <div style={{ display: "grid", gap: 8 }}>
              {city.npcs.map((n) => (
                <div key={n.id} style={{ display: "flex", gap: 8, alignItems: "center" }}>
                  <img src={n.portrait} alt={n.name} style={{ width: 44, height: 44, borderRadius: 8, objectFit: "cover" }} />
                  <div>
      ...
      <section style={{ padding: 24 }}>
        <img src={city.bigMap} alt="city map" style={{ width: "100%", borderRadius: 12, border: "1px solid rgba(255,255,255,0.1)" }} />
      </section>
    </main>
  );
}
    ...
    <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "6px 0", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
      ...
      {avatar ? (
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <img src={avatar} alt="avatar" style={{ width: 24, height: 24, borderRadius: 4, objectFit: "cover" }} />
          <div>{value}</div>
        </div>
      ) : (
        <div>{value}</div>
